...
from app.modules.persona.schemas import (
    PersonaCreate,
    PersonaUpdate,
    PersonaResponse,
    PersonaPaginatedRead,
    PersonaFiltro,
)
...
class PersonaService:
    ...
    def _get_or_404(self, uow: UnitOfWork, persona_id: int):
        ...
        if not persona:
            raise HTTPException(
                status.HTTP_404_NOT_FOUND, f"Persona con id {persona_id} no encontrada"
            )
        return persona
        ...
        with UnitOfWork(self._session) as uow:
            if uow.personas.exists_by_dni(data.dni):
                raise HTTPException(
                    ...
                    f"Ya existe una persona con DNI {data.dni}",
                )
            persona = uow.personas.add_from_schema(data)
            result = PersonaResponse.model_validate(persona)
        return result

    def get_all(self, filtro: PersonaFiltro) -> PersonaPaginatedRead:
        with UnitOfWork(self._session) as uow:
            personas, total = uow.personas.get_filtered(filtro)
    ...
    def get_by_dni(self, dni: str) -> PersonaResponse:
        with UnitOfWork(self._session) as uow:
            persona = uow.personas.get_by_dni(dni)
            if not persona:
                raise HTTPException(
                    status.HTTP_404_NOT_FOUND,
                    f"Persona con DNI {dni} no encontrada",
                )
        ...
        with UnitOfWork(self._session) as uow:
            ...
            if "dni" in update_data and update_data["dni"] != persona.dni:
                if uow.personas.exists_by_dni(update_data["dni"]):
                    raise HTTPException(
                        status.HTTP_409_CONFLICT,
                        f"Ya existe una persona con DNI {update_data['dni']}",
                    )
            for field, value in update_data.items():
                setattr(persona, field, value)
            persona.updated_at = __import__("datetime").datetime.now(
