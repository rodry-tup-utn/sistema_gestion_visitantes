...
export default function ServicioAmbulatorioForm({
  isOpen,
  onClose,
  onSubmit,
  initialData,
  ...
  const { values, handleChange, reset } =
    useForm<CreateServicioAmbulatorioPayload>({
  ...
  return (
    <Modal
      ...
      title={
        initialData
          ? "Editar Servicio Ambulatorio"
          : "Nuevo Servicio Ambulatorio"
      }
      footer={
        <>
    ...
    >
      <div className="space-y-3">
        ...
        <div>
          ...
          <input
            value={values.ubicacion_interna ?? ""}
            onChange={handleChange("ubicacion_interna")}
            className="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
          />
        </div>
        <div>
