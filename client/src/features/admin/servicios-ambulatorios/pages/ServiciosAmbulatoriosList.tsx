...
export default function ServiciosAmbulatoriosList() {
  ...
  const [editingServicio, setEditingServicio] = useState<ServicioAmbulatorio | null>(null);
  const limit = 20;
  const { data, isLoading } = useServicios(page * limit, limit);
  const { data: searchData, isLoading: searchLoading } = useSearchServicios(search, page * limit, limit);
      ...
      <div className="mx-auto max-w-4xl px-4 py-6">
        ...
        {isLoading || searchLoading ? (
          ...
          <>
                ...
                <Card key={s.id}>
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      ...
                      {s.ubicacion_interna && <p className="text-sm text-muted">{s.ubicacion_interna}</p>}
                    </div>
            ...
            <Pagination page={page} total={list?.total ?? 0} limit={limit} onPageChange={setPage} label="servicios" />
          </>
        )}
      </div>
      {isAdmin && (
