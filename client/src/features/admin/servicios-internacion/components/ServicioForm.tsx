...
export default function ServicioForm({ isOpen, onClose, onSubmit, initialData }: Props) {
  const { values, handleChange, reset } = useForm<CreateServicioInternacionPayload>({
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={initialData ? "Editar Servicio de Internación" : "Nuevo Servicio de Internación"}
    ...
    >
      <div className="space-y-3">
        ...
        <div>
          <label className="block text-xs font-medium text-gray-600">Bloque / Piso</label>
          <input value={values.bloque_piso ?? ""} onChange={handleChange("bloque_piso")} className="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" />
        </div>
      </div>
    </Modal>
  );
