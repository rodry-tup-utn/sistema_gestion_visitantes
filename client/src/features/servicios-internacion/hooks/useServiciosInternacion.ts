...
export function useServicios(
  offset = 0,
  limit = 20,
): UseQueryResult<ServicioInternacionPaginated> {
  return useQuery({
    queryKey: [...KEY, { offset, limit }],
    queryFn: () => svc.getServicios(offset, limit),
  });
}
...
export function useCreateServicio() {
  ...
  return useMutation({
    mutationFn: (data: CreateServicioInternacionPayload) =>
      svc.createServicio(data),
